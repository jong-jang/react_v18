...
/*
	...
	2. 추후  동적인 데이터가 필요할때마다 다시 서버쪽에  요청해서 전체화면을 full load (화면 깜빡임)
	...
	2. 자바스크립트 파일로드 (React파일)
	3. 리액트 컴포넌트가 로드 (Data fetching)
	4. 컴포넌트 해석후 렌더링 시작
	5. 최종화면에 동적 DOM생성 (1~4단계까지는 빈화면이 redering)
	...
	- 이전 버전까지는 클라이언트 컴포넌트에서만 제한적으로 동작되는 기술이었으나 18버전부터는 SSR방식의 컴포넌트에서도  적용가능하도록 개선
	...
	- Suspense로 특정 컴포넌트를 동기화시키기 위해서는 내부에 promise객체 상태(pending, fullfiled, rejected)를 추척할 수 있어야됨
*/
